...
Supporta sia firme embedded che detached
"""
...
def find_original_pdf(p7m_path):
    """
    Cerca il PDF originale associato al file p7m
    ...
    pdf_path = Path(str(p7m_path)[:-4])  # Rimuove .p7m
    ...
    try:
        ...
        if pdf_content is None:
            return False, None, None, (
                "Firma detached: PDF originale non trovato. "
                f"Assicurati che il file '{Path(p7m_file_path).stem}' esista nella stessa directory."
            )
        ...
        if is_cades:
            if is_detached:
                firma_tipo = "CAdES-BES Detached (firma separata dal documento)"
            else:
                firma_tipo = "CAdES-BES Embedded (firma incorporata)"
        else:
            if is_detached:
        ...
        return False, None, None, f"Errore generico: {str(e)}"
...
def process_directory(input_dir, output_base_dir):
    ...
    if errors_detail:
        ...
        print(f"\n💡 Suggerimento:")
        print(f"   Se hai firme detached (CAdES), assicurati che i PDF originali")
        print(f"   siano nella stessa directory dei file .p7m con lo stesso nome.")
        print(f"   Esempio: 'documento.pdf' e 'documento.pdf.p7m'")
...
Per firme detached:
  Il PDF originale deve essere nella stessa directory del file .p7m
  Esempio: documento.pdf e documento.pdf.p7m
...
Struttura output:
  ...
  └── YYYYMMDD_HHMMSS/
    ...
    print("Supporto: PKCS#7 e CAdES (embedded e detached)")
    print("=" * 80)
    print()
